---
title: "Kaggle-Russian Housing: Importing data and analysing year"
author: "Maria"
date: "May 19, 2017"
...
### Is price increasing with year+month?

```{r}
dtotal[!is.na(dtotal$price_doc),] %>% 
  group_by(yearmonth) %>% 
  summarize(mean_build_price=mean(price_doc)) %>%
  ggplot(aes(x=as.factor(yearmonth), y=mean_build_price))+
  geom_point(color="darkred",size=3)+
  geom_smooth(method="lm",color='darkgrey')+
  scale_y_continuous(labels = dollar)+
  labs(x="Year+Month",y="Mean Price",title="Price per Year+Month relationship")+
  theme_minimal()+
  theme(axis.text.x=element_text(angle=90, hjust=1)) # hjust adjust the text to the graph
```

There is probably some 'yearmonth' outliers where the price is extremely high or extremely low compared to the natural tendency.
